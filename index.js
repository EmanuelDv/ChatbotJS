...
client.on("qr", (qr) => {
    console.log("Escanea el QR usando este texto:");
    console.log(qr);
    console.log("1. Copia el texto de arriba (empezando con '2@...').");
    console.log("2. Pégalo en https://www.qr-code-generator.com/");
    console.log("3. Escanea la imagen generada con WhatsApp.");
});

client.on("ready", () => {
    console.log("¡Pamii-bot conectado a WhatsApp! Listo para recibir mensajes.");
});
...
const menus = {
    terminos: `¡Hola! 👋 ¡Bienvenid@! Gracias por ponerte en contacto con nosotros. Antes de iniciar, es necesario que aceptes los términos y condiciones de WhatsApp. Si quieres conocer más, ingresa aquí: ${process.env.TERMS_LINK || '[link]'}
    \nPara continuar elige:
    \n1. Acepto
    \n2. No acepto`,
    principal: `¡Hola! 👋 ¡Bienvenid@ a EstoEsPamii! Soy tu asistente virtual, list@ para darte una mano. 😉  
...
const stageHandlers = {
    inicio: async (message) => {
        await message.reply(menus.terminos);
        estadosConversacion[message.from] = { nivel: "terminos", timeoutId: null };
    },
    terminos: async (message, estado) => {
        const opcion = message.body.trim();
        const opciones = {
            "1": { nivel: "principal", respuesta: menus.principal },
            "2": { nivel: null, respuesta: "Acepta los términos y condiciones para desplegar el chatbot." },
        };
        const seleccion = opciones[opcion] || { respuesta: "Opción no válida. Selecciona 1 para aceptar o 2 para rechazar." };
        await message.reply(seleccion.respuesta);
        if (seleccion.nivel === null) {
            ...
            estadosConversacion[message.from] = { nivel: seleccion.nivel, timeoutId: null };
        }
    },
    principal: async (message, estado) => {
        const opcion = message.body.trim();
